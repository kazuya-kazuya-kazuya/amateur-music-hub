...
## Phase 7: 統計ダッシュボード実装（再生数・いいね数・コメント数グラフ）
- [x] server/routers/stats.ts: 統計情報取得エンドポイント追加（楽曲ごとの再生数・いいね数・コメント数）
- [x] pages/MyStats.tsx: 統計ダッシュボードページ実装（Recharts使用）
- [x] MyMusicページに「統計を見る」ボタン追加
- [x] グラフコンポーネント：折れ線グラフ（時系列再生数）、棒グラフ（楽曲別いいね数）、円グラフ（ジャンル別再生数）
- [x] 統計情報の集計ロジック実装
- [x] Vitestテスト追加（stats.getTrackStats） - 28件全てパス
- [x] 最終チェックポイント保存
